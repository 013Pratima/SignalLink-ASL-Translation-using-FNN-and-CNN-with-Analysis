...
def backward_prop(Z1, A1, Z2, A2, W1, W2, X, Y, num_classes):
    m = X.shape[1]
...
def gradient_descent(X_train, Y_train, X_val, Y_val, alpha, iterations, batch_size):
    ...
    num_classes = np.max(Y_train) + 1  # Ensure num_classes is correctly set
    W1, b1, W2, b2 = init_params(X_train.shape[0], num_classes)
    ...
    for i in range(iterations):
        for batch_start in range(0, num_samples_train, batch_size):
            ...
            dW1, db1, dW2, db2 = backward_prop(Z1, A1, Z2, A2, W1, W2, X_batch, Y_batch, num_classes)
...
def train_model2(lr=0.01, epoch=20, batch_size=32):
