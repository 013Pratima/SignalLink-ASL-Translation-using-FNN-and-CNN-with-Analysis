...
def train_model(x_train, y_train, x_test, y_test, epochs=50, batch_size=32,lr=0.001):
...
def metrices1(y_pred, y_test):
    # Convert y_test from one-hot encoded to label encoded format
    y_test_labels = np.argmax(y_test, axis=1)
    acc = metrics.accuracy_score(y_test_labels, y_pred)
    ...
    f1 = metrics.f1_score(y_test_labels, y_pred, average='weighted')
...
def main():
    ...
    history, model = train_model(x_train, y_train, x_test, y_test, epochs=50, batch_size=32, lr=0.001)
    y_pred = predict_model(model, x_test)
    confusion_matrix_fig = print_confusion_matrix(y_pred, y_test)
    line_fig = line_plot(history, epoch=50)
    model.save("./model/cnn.h5")
    print("Model Saved")
    acc, precision, recall, f1 = metrices1(y_pred, y_test)
    fig = acc_plot(history, epoch=50)
